...
def main():
	...
	junctions = read_junctions(args[0] + '/SJ.out.tab', junction_reads, args[1])
	...
	with open(file, 'r') as f:
		for line in f:
			...
			for tmp in reads[key]:
				tmpdict[tmp[0]] = ''
			if len(tmpdict) < int(min_reads):
				continue
			print key,"\t",len(tmpdict)
			#n = n + 1
...
# store reads id that mapped to human_epcific_exon to a dict
	...
	for line in d.stdout.readlines():
		...
		ele = line.split()
		rs = int(ele[7]) + 1	# 7
		readsid = str(rs)+'_'+ele[9].split("/")[0] # format xMyNzM # 9
		if ele[3] == 'test':
			continue
		exonid[readsid] = ''
	retval = d.wait()
