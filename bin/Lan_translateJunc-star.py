...
# store reads id that mapped to human_epcific_exon to a dict
	exonid = {}
	d = subprocess.Popen('samtools view -b -q 255 ' + sam_fn + ' | bedtools intersect -a /u/home/y/ybwang/scratch/HumanSpecificExon/data/Lan.sorted.bed -b stdin -wa -wb -split -sorted', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
	for line in d.stdout.readlines():
		line = line.rstrip()
		ele = line.split()
		rs = int(ele[5]) + 1
		readsid = str(rs)+'_'+ele[7].split("/")[0] # format xMyNzM
		if ele[3] == 'test':
			continue
		exonid[readsid] = ''
	retval = d.wait()
...
def junctionFilter(junctionReads, out):
	...
	p = subprocess.Popen('bedtools coverage -a ' + tmpbed + ' -b /u/home/y/ybwang/scratch/HumanSpecificExon/data/Lan.sorted.bed', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
	for line in p.stdout.readlines():
                line = line.rstrip()
		ele = line.split()
		if ele[6] != '0':
